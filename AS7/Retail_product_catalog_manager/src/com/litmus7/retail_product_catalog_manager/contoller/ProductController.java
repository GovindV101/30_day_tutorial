...
public class ProductController {
    ...
    private String validateProductInput(Product product, boolean isUpdate) {
        if (product == null) return "Product cannot be null.";
        if (isUpdate && product.getProductId() <= 0) return "Product ID must be a positive integer.";
        if (product.getName() == null || product.getName().trim().isEmpty()) return "Product name cannot be empty.";
        if (product.getCategory() == null || product.getCategory().trim().isEmpty()) return "Product category cannot be empty.";
        if (product.getPrice() < 0) return "Price cannot be negative.";
        if (product.getStockQuantity() < 0) return "Stock quantity cannot be negative.";
        return null; 
    }
